...
export async function startWorkflowExecution(
    ...
    if (rule && rule.delaySeconds > 0) {
        logger.info(`规则 "${rule.name}" 配置了 ${rule.delaySeconds} 秒延迟，订单 ${context.orderId} 等待中...`)
        await new Promise(resolve => setTimeout(resolve, rule.delaySeconds * 1000))
        logger.info(`订单 ${context.orderId} 延迟完成，开始执行流程`)
    }
